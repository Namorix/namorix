...
#### ✅ GeoIP Database Management — Settings tab (2026-08-09)

- [x] **FrontgateSettings tab**: tab `settings` trong `Frontgate.tsx` → `FrontgateSettings.tsx` — `NmxSettingsSection`/`NmxSettingsCard`/`NmxSettingsRow` hiện GeoIP status (exists badge, DatabaseType, Build date, File size), upload field + rollback button.
- [x] **GeoIP status API**: `GET /api/frontgate/geoip` — `GeoIpService.GetStatus()` đọc meta current + backup (`.bak`), trả `GeoIpStatus` mở rộng: `HasBackup`, `BackupFileSize`, `BackupDatabaseType`, `BackupBuildEpoch`. Helper `ReadMeta` dùng `DatabaseReader.Metadata` — `BuildDate`/`BinaryFormatMajorVersion` (MaxMind public API, **không** phải `BuildEpoch` internal).
- [x] **Upload API**: `POST /api/frontgate/geoip` (multipart, `RequestSizeLimit(50MB)`, `IFormFile`) — `TryUpdateDatabase` probe validate mmdb trong `MemoryStream` trước, copy DB hiện tại → `.bak` **trước** khi overwrite (trong lock), reset reader. Error codes `FG_GEOIP_FILE_REQUIRED` / `FG_GEOIP_INVALID`.
- [x] **415 fix — `ExemptPaths.NonJsonBody`**: thêm `/api/frontgate/geoip` — `JsonErrorMiddleware` bỏ qua content-type check (multipart body) cho endpoint này.
- [x] **Upload progress — XHR**: `nmxHttp.formUpload<T>(body, onProgress)` trong `RequestBuilder` dùng `XMLHttpRequest.upload.onprogress` (fetch không có upload progress event), giữ `withCredentials` + `x-csrf-token` + `x-device-fingerprint`, parse `ApiResponse<T>` như `json()`. `NmxFileInput` thêm prop `progress?: number | null` để hiện progress bar trong drop zone.
- [x] **Rollback API**: `POST /api/frontgate/geoip/rollback` — `RollbackDatabase` probe validate `.bak`, copy về `_dbPath`, **`File.Delete(.bak)`** (consume backup → `HasBackup=false` → nút rollback ẩn), reset reader. Error `FG_GEOIP_ROLLBACK_FAILED`.
- [x] **Rollback UI**: nút Rollback (warning, `shouldRender={status?.hasBackup === true}`) → `NmxAlertDialog` (warning, `loading`) hiện so sánh bản mới vs bản cũ qua `NmxMetaList`/`NmxMetaItem` (Version/Build/Size mỗi bên) → confirm gọi `rollbackGeoIp()` → refetch status + toast.
- [x] **Frontend controller**: `GeoIpStatus` interface + backup fields, `getGeoIpStatus()`, `uploadGeoIp(formData, onProgress)`, `rollbackGeoIp()`; `ApiFrontgateRoutes.geoIp` + `geoIpRollback`.
- [x] **i18n**: `pages.settings.*` (sections/fields/actions/errors) + `rollbackDialog.*` (title/description/confirm/cancel/current*/backup*) + error map `FG_GEOIP_*` trong `Frontgate.types.ts`.
- [ ] **Bug pending — typo `geoIpFileReuired`**: `Frontgate.types.ts` map `FG_GEOIP_FILE_REQUIRED → "addon.frontgate.errors.geoIpFileReuired"` (thiếu `q`) — i18n lookup fail, key render nguyên bản khi lỗi này xảy ra. Sửa thành `geoIpFileRequired` (en.json đã có key đúng).
