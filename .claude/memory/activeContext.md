...
- **Access token = JWT RS256 (Phase 1c):** `OAuthService` inject `NmxAddonTokenSigner`, `ExchangeCodeAsync`/`RefreshAddonTokenAsync` trả `signer.Sign(userId, clientId, ttl)` thay GUID — row `OAuthTokens` vẫn ghi (audit + revoke tra được qua `FindAsync`). `AccessTokenTtl` = `TimeSpan.FromSeconds(OAuth.AddonToken.AccessTokenTtlSeconds)`.
- **TTL một nguồn duy nhất — `OAuth.AddonToken.AccessTokenTtlSeconds = 900`:** trước đó **6** chỗ hardcode `3600` (không phải 4 như kế hoạch ghi) — thêm `OAuthController.cs:66` (`token`/authorization_code) và `:120` (`token/refresh`) ngoài `AddonChannelService` ×2 và `OAuthService` ×2. Đây là bug thật: JWT `exp` 900s mà `expires_in` trả 3600 → addon chờ thêm 45 phút mới refresh, mọi request trong khoảng đó fail. Machine token (`client_credentials`) **giữ** TTL riêng (`OAuthController.cs:89`, `OAuthService.cs:222`).
- **`NmxAddonTokenSigner` (Namorix.Server, KHÔNG phải Core):** Core là addon SDK nên không được mang code ký/private key. RSA 2048, PKCS#8 PEM `{DataDir}/oauth-signing.pem` mode 600, `kid` = base64url(SHA-256(SPKI)). Khoá đọc hỏng → ném lỗi chỉ rõ cách xử lý, không âm thầm regenerate.
- **Phát khoá qua gRPC `GetJwks` (Phase 1b):** thêm RPC vào `addon_channel.proto`, trả `kid` + `public_key_pem` (SPKI/PEM). Không HTTP endpoint, không cache đĩa — addon không kết nối được desktop thì **không** được verify gì (DG1).
- **Gate khả dụng fail-closed (Phase 1b):** `AddonChannelClient.IsConnected` từ `_call != null` → `_streamUp` volatile, chỉ bật khi desktop gửi `handshake`, tắt khi receive loop kết thúc kể cả stream đóng sạch; keepalive ping 15s/10s policy `Always` để desktop chết không sạch lộ ra trong vài giây. `AddonChannelService.Connect` ghi `Handshake()` vô điều kiện trước mọi thứ có thể fail.
- **Hợp đồng lỗi refresh (Phase 0.5):** gRPC `Unauthenticated` bị overload → phân loại qua trailer `OAuth.Trailer.ErrorCode`. `AddonSessionMiddleware` chỉ xoá session khi gặp `theft_detected`/`invalid_grant`; lỗi transient (desktop restart, channel chưa start, timeout) → **503 giữ session**. `AddonSessionLockRegistry` (mới) + đọc lại session trong lock → 2 request đồng thời không cùng refresh (lần rotate thứ hai bị coi là theft, xoá sạch chain).
- **Migration:** `20260914020654_AddOAuthRefreshTokenUserId` thêm cột `UserId` (phải `make db-update`). `20260914025244_WidenOAuthTokenId` **Up/Down rỗng** — SQLite map mọi `MaxLength` sang `TEXT` nên `200 → 1024` không tạo delta; chỉ cần để advance snapshot (xoá đi → `PendingModelChangesWarning`).
- **Lỗ có sẵn, chưa sửa:** `ValidateTokenAsync` không phân biệt grant type → user access token cũng được nhận làm auth cho gRPC channel. Row refresh token cũ có `UserId = 0` → JWT `sub = "0"`.
- Versions: Namorix.Core 0.62.1 → 0.63.0 / Namorix.Server 0.80.1 → 0.81.0 (@namorix/core/ui/styles/frontend không đổi — không bump).
...
- **Settings → Docker tab (frontend 0.92.0):** `Desktop domain` từ Settings → System chuyển sang tab **"Docker"** mới (admin-only, `ADMIN_TABS = ["system","docker"]`) kèm 2 field mới `Container name`/`Network name` (`SettingsDocker.tsx`). Backend tách API: `GET/PUT /api/settings/docker` (`desktopDomain`/`containerName`/`networkName`), `GET /api/settings/system`/`PUT` bỏ `desktopDomain`; `SettingsService` +`Get/SetDesktopContainerName` +`Get/SetDesktopNetworkName` (upsert DB, fallback `BackendConfig.ContainerName/NetworkName`), inject `IOptions<BackendConfig>` — **WIP**: container/network name chưa được consume khi spawn (đang host-mode). `appsettings.json` `Backend.NetworkName` `namorix-net` → `namorix_default`.
- **Namorix.Core 0.62.0:** `Constants/Settings.cs` +`SettingKeys.DesktopContainerName/DesktopNetworkName`; `DevViteReverseProxyExtensions` bỏ host match (`["localhost","127.0.0.1"]`) → catch-all mọi Host (proxy tới Vite qua IP/hostname LAN).
- **@namorix/ui 0.51.0 / @namorix/styles 0.60.0:** icon `docker` cho tab Docker — `NmxIconFontSymbol.DOCKER` (`ic-docker`) + icomoon glyph `$ic-docker: "\e947"`.
- **@namorix/core 0.67.4 / settings addon 1.2.0:** `apiRoutes.ts` +`ApiSettingsRoutes.docker` (`/api/settings/docker`); `version.ts` `NmxAddonVersions.settings` 1.1.0 → 1.2.0.
- Versions: Namorix.Server 0.79.0 → 0.80.0 / Namorix.Core 0.61.0 → 0.62.0 / @namorix/core 0.67.3 → 0.67.4 / @namorix/ui 0.50.0 → 0.51.0 / @namorix/styles 0.59.0 → 0.60.0 / frontend 0.91.0 → 0.92.0.

### 2026-09-07 — Desktop Domain setting + gRPC config-update push cho addon OAuth redirect + DockerService tmpfs /tmp (Namorix.Server 0.79.0 / Namorix.Core 0.61.0 / frontend 0.91.0)

- **Desktop Domain feature:** Addon OAuth authorize URL trước build từ `config.DesktopApiUrl` = `http://host.docker.internal:5000` → browser (mở trong app) không resolve được `host.docker.internal`. Desktop giờ có setting `desktop_domain` (`SettingKeys.DesktopDomain`) — value full origin `http(s)://host:port` desktop truy cập được từ LAN — push **động qua gRPC** cho addons; backend→backend vẫn giữ `host.docker.internal`.
- **Namorix.Core 0.61.0:** NEW `Grpc/DesktopConfigMessage.cs` — message type `config-update`, payload `{"desktopDomain":"http(s)://host:port"}` (helper build/parse). `AddonChannelClient` +`BrowserOrigin` (capture từ config-update); `AddonSessionAuthService.BuildLoginUrlAsync` fallback `channel.BrowserOrigin ?? config.DesktopApiUrl`.
- **Namorix.Server 0.79.0:** `SettingsService` +`GetDesktopDomain`/`SetDesktopDomain` (upsert thẳng DB); `SettingsController` GetSystem/SetSystem +`DesktopDomain`, sau Set gọi `AddonChannelManager.BroadcastAsync(DesktopConfigMessage.ConfigUpdate(...))`; `AddonChannelService.Connect` push config-update best-effort ngay sau khi `ResponseStream` sẵn sàng (addon nối lại vẫn nhận được config). **DockerService tmpfs:** `CreateContainerAsync` +`Tmpfs {"/tmp" = "rw"}` — fix crash addon container read-only rootfs khi DataProtection tạo key-ring (`Path.GetTempFileName()` → `Read-only file system : '/tmp/'`).
- **frontend 0.91.0:** `SettingsSystem.tsx` +section "Domain" (`NmxSettingsSection` + `NmxFormInput` Desktop domain — load/save/reset-on-fail); `settings.controller.ts` types +`desktopDomain`; `en.json` +3 key `addon.settings.system.*`.
- Versions: Namorix.Core 0.60.0 → 0.61.0 / Namorix.Server 0.78.4 → 0.79.0 / frontend 0.90.2 → 0.91.0 (@namorix/core/ui/styles không đổi — không bump).
