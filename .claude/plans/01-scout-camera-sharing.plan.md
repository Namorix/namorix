...
> **Tiến độ:** **DG-S1 ✅ chốt 2026-09-16 — đi hướng B (thêm RPC `SearchUsers`), đã code + build xong**, xem mục 8. · **Phase 1 ✅ 2026-09-16 — `CameraShare` + migration `AddCameraShare`, build sạch; query chưa đổi (đúng như plan)**. · Phase 2, 4–5 chưa làm. Toàn bộ mục "Điều tra được từ code" dưới đây là đọc thật, có file:line.
>
...
### Phase 5 — Ngữ nghĩa revoke (chỉ khi DG-S4 chọn "ngắt ngay")
- Thêm `UserId` vào `RtcViewerSession` để `WebRtcRelayService` đóng được đúng phiên của user bị revoke.

### Phase 6 — Docs + version
- namorix MINOR: `Namorix.Core` `0.70.0 → 0.71.0`, `Namorix.Server` `0.87.0 → 0.88.0` (proto có thêm `SearchUsers` + `GetUsers`).
...
- Cập nhật `progress.md` cả hai repo, `FLOW.md` mục addon, `backend/README.md`.
