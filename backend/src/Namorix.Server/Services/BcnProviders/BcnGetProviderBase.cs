...
public abstract class BcnGetProviderBase(IHttpClientFactory httpFactory) : IBcnProviderClient
{
    ...
    protected abstract string BuildUrl(string hostname, BcnProviderConfig config,
    ...
    {
        var url = BuildUrl(hostname, config, ipv4, ipv6);
    ...
    public async Task<BcnTestResult> TestAsync(string hostname, BcnProviderConfig config,
        IPAddress? ipv4, IPAddress? ipv6, CancellationToken ct)
    {
        try
        {
            var result = await UpdateAsync(hostname, config, ipv4, ipv6, ct);
