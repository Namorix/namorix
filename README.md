...
4. **M4** — External addon system: Docker lifecycle, OAuth2 (client_credentials + private_key_jwt + authorization_code + PKCE), gRPC bidirectional streaming, addon catalog sync, standalone mode, web UI ✅
    ...
    - **Warden addon**: host-level firewall — rules CRUD + iptables/nftables enforcement (Phase 1-2 ✅ — event publishing từ AcmeChallenge/Scan404, threshold engine + auto-ban theo security profile Low/Medium/High/Custom), Herald notifications (Phase 4 ✅ — `warden:ruleApplied`/`ruleRemoved` cho admin), Overview/Activity/Rules dashboard tabs + stats realtime qua SignalR (Phase 3 ✅), Activity Clear (xóa toàn bộ security events qua confirm dialog) + search theo IP, **audit trail đầy đủ** (rule lifecycle — auto-ban/manual/expiry — ghi cả vào Activity log, không chỉ Notification)
